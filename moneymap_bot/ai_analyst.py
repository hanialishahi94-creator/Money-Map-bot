...
async def rewrite_signal(raw_signal: str) -> str:
    """
    پیام فوروارد‌شده رو آنالیز و فرمت می‌کنه:
    - اگه سیگنال خرید/فروشه → فرمت شکیل سیگنال (بدون لوریج)
    - اگه تحلیل/خبر/چیز دیگه‌ست → فرمت متناسب با ماهیتش
    """
    prompt = f"""متن زیر یه پیام از یه کانال ترید است. اونو آنالیز کن و **فقط** خروجی فرمت‌شده رو بده — هیچ توضیح، سلام یا امضا اضافه نکن.
...
## اگه پیام یه سیگنال خرید یا فروش بود:

خروجی دقیقاً به این شکل باشه (هر بخش که نبود حذف کن، چیزی اضافه نکن):

🟢 سیگنال خرید [نام فارسی دارایی]    ← اگه LONG/BUY بود
🔴 سیگنال فروش [نام فارسی دارایی]    ← اگه SHORT/SELL بود

📌 ورود: [محدوده یا قیمت ورود]
...
قوانین سیگنال:
- نام دارایی فارسی: Bitcoin→بیتکوین، BTC→بیتکوین، ETH→اتریوم، Gold/XAU→طلا، XAUUSD→اونس طلا، EUR→یورو، USDT→تتر
- اعداد کریپتو (BTC، ETH و سایر ارزها): اعشار اضافه حذف کن — مثلاً ۶۵۹۶۶.۰۰۰۰۰۰۰۰ → ۶۵,۹۶۶ | ۶۴۴۰۹.۱۸۶۱۷۵۰۰ → ۶۴,۴۰۹.۱۹
- اعداد طلا و فارکس: حداکثر ۲ رقم اعشار نگه دار
- لوریج، اهرم، Leverage، LOT → حذف کامل
- تارگت‌هایی که نیستن رو ننویس
- فقط فارسی

---

## اگه پیام تحلیل بازار بود (نه سیگنال مستقیم):

📊 تحلیل | [نام فارسی دارایی یا بازار]

[خلاصه فارسی تحلیل — مستقیم و حرفه‌ای، حداکثر ۴ خط]

---

## اگه پیام خبر یا رویداد اقتصادی بود:

📰 خبر بازار

[خلاصه فارسی خبر — یک یا دو جمله کوتاه]

---

## اگه پیام هیچ‌کدام از موارد بالا نبود:

💬 [خلاصه فارسی محتوا — یک جمله]"""
