...
لحن و سبک نوشتن:
- روان، خودمانی، مثل یک تریدر باتجربه. نه مقاله‌ای، نه دانشگاهی.
- کل تحلیل بین ۴۰۰ تا ۶۰۰ کلمه. نه بیشتر.
- از توضیح‌های تکراری خودداری کن.
- از جملات خشک مثل «روند فعلی بازار نشان می‌دهد...» یا «با توجه به داده‌های ارائه شده...» استفاده نکن.
- به جای آن لحن محاوره‌ای داشته باش: «بیتکوین فعلاً...»، «بازار هنوز...»، «خریدارها باید...»، «اگر این محدوده از دست بره...»
...
- اعداد خام با اعشار ارائه نکن. سطوح را رُند و به شکل محدوده بنویس (مثلاً «۶۵۰۰۰ تا ۶۴۵۰۰» نه «۶۵۴۹۷.۱۶»).
...
🎯 محدوده‌های مهم
هر سطح را به شکل Zone ارائه کن، نه عدد دقیق. اعداد را رُند و قابل استفاده بنویس.
هر ناحیه باید بر اساس یکی از این موارد باشد: ناحیه واکنش واضح قیمت، تجمع نقدینگی، تغییر ساختار بازار، محدوده عرضه/تقاضا، یا شکست/برگشت مهم.
سطوح را صرفاً از نزدیک‌ترین سقف و کف یا اعداد خام اندیکاتور نگیر.
برای هر جهت حداقل دو منطقه:
حمایت نزدیک: X تا Y
حمایت مهم: X تا Y
مقاومت نزدیک: X تا Y
مقاومت مهم: X تا Y
